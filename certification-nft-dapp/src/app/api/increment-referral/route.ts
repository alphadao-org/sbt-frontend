...
export async function POST(request: Request) {
  try {
    ...
    if (!referrerAddress) {
      return NextResponse.json({ success: false, message: "Missing referrerAddress" }, { status: 400 });
    }

    const ok = await incrementReferralCount(referrerAddress);
    if (!ok) return NextResponse.json({ success: false, message: "Failed to increment" }, { status: 500 });
